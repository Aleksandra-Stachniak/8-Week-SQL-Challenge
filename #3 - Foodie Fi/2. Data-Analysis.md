...
SELECT COUNT(DISTINCT customer_id) AS churned_customers,
       ROUND(100.0 * COUNT(DISTINCT customer_id) / 
            (SELECT COUNT(DISTINCT customer_id) FROM foodie_fi.subscriptions), 0) AS churn_percentage
...
SELECT   next_plan as plan_name,
         ...
         ROUND(100.0 * COUNT(DISTINCT customer_id) / 
               (SELECT COUNT(DISTINCT customer_id) FROM foodie_fi.subscriptions), 0) AS conversion_percentage
FROM     customer_journeys
...
SELECT   plan_name,
         ...
         ROUND(100.0 * COUNT(DISTINCT customer_id) /
              		(SELECT COUNT(DISTINCT customer_id) FROM foodie_fi.subscriptions), 1) AS percentage
FROM     next_dates
...
SELECT ROUND(AVG(apc.annual_start_date - tpc.trial_start_date)) AS avg_conversion_days
FROM   trial_plan_cte tpc
JOIN   annual_plan_cte apc ON tpc.customer_id = apc.customer_id
WHERE  apc.annual_start_date > tpc.trial_start_date;
```
